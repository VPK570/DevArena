        ...
        <div className="bg-[#0E0E11] p-8 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-4 mb-2">
              <span className={`font-headline text-[10px] tracking-widest border px-2 py-1 bg-opacity-5 ${themeColor} ${borderColor} ${bgColor === 'bg-[#00F0FF]' ? 'bg-[#00F0FF]' : bgColor === 'bg-error' ? 'bg-error' : 'bg-yellow-400'}`}>
                {verdict === "Pass" ? 'TRANSMISSION_STABLE' : 'TRANSMISSION_ANOMALY'}
              </span>
              <span className="h-px flex-1 bg-surface-container-high"></span>
            </div>
            
            <h1 className={`text-6xl font-black font-headline tracking-tighter leading-none mb-4 ${themeColor}`}>
              {title}
            </h1>
            
            <p className="text-[#B3B7CF] font-headline text-sm tracking-widest uppercase mb-8">
              {verdict === "Pass" ? "FLAWLESS EXECUTION" : "SEQUENCE ITERATION REQUIRED"}
            </p>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className={`border-l-2 ${borderColor} pl-4`}>
                ...
                <div className={`text-xl font-bold ${themeColor}`}>{formatTime(timeTaken)}</div>
              </div>
              <div className={`border-l-2 ${borderColor} pl-4`}>
        ...
        <div className="bg-[#131316] flex flex-col border-l border-[#00F0FF]/15">
          <div className="p-4 border-b border-[#00F0FF]/15 flex justify-between items-center bg-[#1c1b1e]">
            <div className="flex items-center gap-3">
              <span className={`material-symbols-outlined text-sm ${themeColor}`}>psychology</span>
              <h2 className="font-headline text-[10px] tracking-widest text-[#e5e1e5] uppercase">ORACLE_ANALYSIS_V4.2</h2>
            </div>
          </div>
          
          <div className="flex-1 p-6 font-body text-xs overflow-y-auto bg-[#0E0E11]/80 space-y-4">
            <div className={`p-4 border-l-4 ${borderColor} ${bgColor}/10 text-[#B3B7CF] leading-relaxed italic`}>
              &quot;{summary}&quot;
            </div>

            {feedback && feedback.length > 0 && (
              <div className="pt-4">
                <h3 className={`font-headline text-[10px] ${themeColor} mb-4 tracking-[0.2em] uppercase`}>Detailed Diagnostics</h3>
                <div className="space-y-2">
                  {feedback.map((item, idx) => (
                    <div key={idx} className={`flex flex-col p-3 bg-[#1c1b1e] border-r-2 ${borderColor}`}>
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <span className={`material-symbols-outlined text-xs ${themeColor}`}>error</span>
                          <span className={`text-[10px] uppercase font-bold ${themeColor}`}>Line {item.line}</span>
                        </div>
                      </div>
                      <p className="text-[10px] text-[#B3B7CF] leading-tight mb-1"><span className="text-[#e5e1e5]">ISSUE:</span> {item.issue}</p>
                      <p className={`text-[10px] ${themeColor} leading-tight`}><span className="text-[#e5e1e5]">FIX:</span> {item.fix}</p>
                    </div>
                  ))}
                </div>
              </div>
