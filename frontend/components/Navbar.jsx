  ...
  return (
    <header className="flex justify-between items-center w-full px-6 h-14 bg-[#131316] fixed top-0 z-50 border-b border-[#00F0FF]/15">
      <div className="flex items-center gap-8">
        <Link href="/" className="text-xl font-bold tracking-widest text-[#00F0FF] font-headline hover:text-white transition-colors">BATTLE-FRONT</Link>
            ...
            <div className="relative">
              ...
              >
                <div className="flex flex-col items-end">
                  <span className="text-[10px] text-white font-headline tracking-tighter uppercase leading-none">{profile?.username || 'GHOST_OPERATOR'}</span>
                  <span className="text-[9px] text-[#00F0FF] font-headline tracking-widest opacity-70 leading-none mt-1">{profile?.elo_rating || 1200} ELO</span>
                </div>
                <div className="w-8 h-8 bg-[#00F0FF]/10 border border-[#00F0FF]/20 flex items-center justify-center overflow-hidden">
                  {profile?.avatar_url ? (
                    <img alt={profile.username} className="w-full h-full object-cover" src={profile.avatar_url} />
                  ) : (
                    <span className="material-symbols-outlined text-sm text-[#00F0FF]">person</span>
                  )}
                </div>
              </button>
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-[#131316] border border-[#00F0FF]/25 shadow-[0_0_20px_rgba(0,240,255,0.1)] z-[100] animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="p-2 border-b border-[#00F0FF]/10 bg-[#00F0FF]/5">
                    <div className="text-[9px] text-[#00F0FF]/50 font-headline uppercase tracking-widest px-2">Access_Level</div>
                    <div className="text-[11px] text-[#00F0FF] font-headline uppercase px-2">AUTHORIZED_USER</div>
                  </div>
                  <div className="p-1">
                    <Link 
                      href="/profile" 
